    ...
    <form className="w-full max-w-[480px] space-y-5" onSubmit={handleSubmit}>
      ...
      <FormSection title="경험" index="02">
        <OptionSelect
          ...
          onChange={(value) => {
            updateField("careerYears", value);
            if (value === "신입") updateField("career", "");
          }}
          columns={5}
        />
        {formData.careerYears !== "신입" && (
          <TextAreaField
            id="career"
            label="경력 상세"
            value={formData.career}
            onChange={(value) => updateField("career", value)}
            placeholder="담당 업무와 성과를 적어주세요."
          />
        )}
        <TextAreaField
          id="experience"
      ...
      <button
        ...
        className="inline-flex w-full mb-20 items-center justify-center gap-2 rounded-md bg-accent px-4 py-3 text-sm font-semibold text-navy-950 transition active:scale-95 hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-40 disabled:active:scale-100"
      >
        {submitting ? "울산의 공고를 분석하는 중…" : "내 역량으로 공고 찾기"}
        <ArrowRight aria-hidden="true" size={16} className="mt-[-3px]" />
      </button>
    </form>
  );
}
