...
const korean = await matchJobs(profile({ major: "국어국문", skills: ["기사작성", "교내신문", "SNS 콘텐츠"], experience: "기사작성과 교내신문, SNS 콘텐츠 제작" }), ulsanJobs);
...
const humanitiesPlanner = await matchJobs(profile({ major: "일반 문과", skills: ["행사기획", "프로젝트 운영"], experience: "학생회 행사기획과 프로젝트 운영" }), ulsanJobs);
...
const noNurseLicense = await matchJobs(profile({ major: "간호", skills: ["환자응대"], experience: "간호학 전공 수업" }), ulsanJobs);
...
const electricalStudent = await matchJobs(profile({ major: "전기전자", skills: ["PLC"], projectExperiences: ["PLC 프로젝트"], experience: "PLC 프로젝트 경험, 경력 0년", preferredLocation: "울산 남구", yearsExperience: 0 }), ulsanJobs);
...
const computerNewcomer = await matchJobs(profile({ major: "컴퓨터공학", skills: ["Java", "Python", "React"], projectExperiences: ["Java와 React 프로젝트"], experience: "Java, Python, React 프로젝트 경험, 경력 0년", yearsExperience: 0 }), ulsanJobs);
...
const northInfrastructure = await matchJobs(profile({ major: "컴퓨터공학", skills: ["인프라운영"], experience: "서버 인프라 운영 프로젝트", preferredLocation: "울산 북구" }), ulsanJobs);
...
const nurseWithLicense = await matchJobs(profile({ major: "간호", certificates: ["간호사 면허"], skills: ["간호사", "환자관리"], experience: "간호사 면허 보유 및 임상실습", preferredLocation: "울산 남구" }), ulsanJobs);
...
const computerExperienced = await matchJobs(profile({ major: "컴퓨터공학", skills: ["Java", "Spring Boot", "웹 애플리케이션 개발"], projectExperiences: ["Java Spring Boot 프로젝트"], experience: "Java 프로젝트 Spring Boot 웹 애플리케이션 개발, 경력 0년", yearsExperience: 0 }), ulsanJobs);
...
const unrelated = await matchJobs(profile({ major: "철학", skills: ["도자기"], experience: "관련 경험 거의 없음" }), ulsanJobs);
